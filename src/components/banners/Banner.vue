<template>
    <div class="bannerSection contrast">
        <div class="firstSection contrast">
            <div>
                <h2 class="headline">ENCONTRE <span class="attention">ARTISTAS</span> QUE TE COMPLETEM NA</h2>
            </div>
            <h1 class="attention-artesania">ARTESANIA</h1>
            <div>
                <h6 class="subline" id="subline">Descubra um universo de expressão artística onde a arte contemporânea
                    brasileira encontra seu lar. Apresentamos uma seleção meticulosa de obras, cada uma contando sua própria
                    história.</h6>
            </div>
            <button class="buttonPrimary">Ver Coleção</button>
            <div class="desktop-only proves">
                <div>
                    <h1 class="boldTitle">200+</h1>
                    <h3 class="lightTitle" id="lightTitle1">Artistas Independentes</h3>
                </div>
                <div>
                    <h1 class="boldTitle">2,000+</h1>
                    <h3 class="lightTitle" id="lightTitle2">Produtos de Alta Qualidade</h3>
                </div>
                <div>
                    <h1 class="boldTitle">30,000+</h1>
                    <h3 class="lightTitle" id="lightTitle3">Clientes Atendidos</h3>
                </div>
            </div>
        </div>
        <div class="secondSection">
        </div>
    </div>
</template>

<script>
export default {
    name: 'BannerComponent'
}
</script>

<style lang="scss" scoped>
.attention-artesania {
    color: #9D3207;
    font-size: 64px;
    font-family: 'Inter';
    font-weight: 800;
    line-height: 1;
}

.bannerSection {
    width: 100%;
    height: 100%;
    display: flex;
    align-items: center;
    background-color: #F3F3DC;
    align-items: stretch;

    @media (max-width: 1060px) {
        flex-direction: column;
    }
}

.firstSection {
    background-color: #F3F3DC;
    height: 100%;
    width: 55%;
    padding: 20px 100px 20px 70px;
    overflow: hidden;

    @media (max-width: 1060px) {
        width: 100%;
        margin: 20px 0px;
    }
}

.firstSection .headline {
    color: #000;
    font-size: 64px;
    font-family: 'Inter';
    font-weight: 800;
    line-height: 1;
}

.firstSection .headline .attention {
    color: #9D3207;
    font-weight: 800;
}

.firstSection .buttonPrimary {
    background-color: #9D3207;
    margin-top: 20px;
    border: 0px;
    border-radius: 62px;
    width: 210px;
    height: 52px;
    color: #fff;
    font-family: 'Inter';
    font-weight: 600;

    @media (max-width: 1060px) {
        width: 100%;
    }
}

.firstSection .proves {
    display: flex;
    margin-top: 37px;
    color: #000;
    justify-content: space-between;
}

.firstSection .boldTitle {
    font-size: 40px;
    font-weight: 600;
}

.firstSection .lightTitle {
    font-size: 16px;

}

.firstSection::before {
    content: "";
    position: absolute;
    right: 47%;
    margin-top: 200px;
    width: 50px;
    height: 50px;
    background-image: url(data:image/png;base64,);
    background-size: cover;
    background-repeat: no-repeat;

    @media (max-width: 1160px) {
        right: 40%;
    }

    @media (max-width: 1060px) {
        right: 10%;
        margin-top: 0px;
    }
}

.secondSection {
    background-image: url('@/assets/img/banner/GirassoisDeVanGogh.png');
    background-size: cover;
    background-position: center;
    height: auto;
    width: 45%;

    @media (max-width: 1060px) {
        height: 115px;
        background-position: top;
        width: 100%;
    }
}

.subline {
    max-width: 545px;
    margin-top: 30px;

    @media (max-width: 1060px) {
        max-width: none;
    }
}

.high-contrast {
    color: white;
    background: black;
}

.high-contrast-text {
    background: black;
    color: white !important;
}</style>